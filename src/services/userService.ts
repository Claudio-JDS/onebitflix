...
function filterLastEpisodesByCourse(episodes: EpisodeInstance[]) {
  ...
  const lastEpisode = episodes.reduce((currentList, episode) => {
    ...
    const listWinthoutEpisodeFromSameCourse = currentList.filter(ep => ep.courseId !== episode.courseId)
    listWinthoutEpisodeFromSameCourse.push(episode)

    return listWinthoutEpisodeFromSameCourse
